...
DOCSTORE_PATH = os.path.join(RAG_INDEX_DIR, "docstore.json")
...
def load_rag_index():
    ...
    try:
        if os.path.exists(FAISS_PATH) and os.path.exists(DOCSTORE_PATH):
            ...
            logger.info(f"RAG: índice cargado con {len(docstore)} chunks")
        else:
            logger.warning("RAG: no se encontró índice (corre index_pdfs.py si usarás PDFs).")
    except Exception as e:
        logger.error(f"RAG: error cargando índice: {e}", exc_info=True)

def embed_query(q: str) -> np.ndarray:
    res = genai.embed_content(model=GEMINI_EMBED_MODEL, content=q)
